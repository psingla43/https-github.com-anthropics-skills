...
The full procedure for running test cases against a skill being developed: spawning the runs, capturing timing data, grading, aggregating into a benchmark, and presenting results to the user via the eval viewer.

This is one continuous sequence — don't stop partway through. Do NOT use `/skill-test` or any other testing skill; everything here uses the skill-creator's bundled scripts and the `eval-viewer/`.
...
2. [Step 1: Spawn all runs in the same turn](#step-1-spawn-all-runs-with-skill-and-baseline-in-the-same-turn)
3. [Step 2: Draft assertions while runs are in progress](#step-2-while-runs-are-in-progress-draft-assertions)
4. [Step 3: Capture timing data as runs complete](#step-3-as-runs-complete-capture-timing-data)
5. [Step 4: Grade, aggregate, and launch the viewer](#step-4-grade-aggregate-and-launch-the-viewer)
6. [Step 5: Read the feedback](#step-5-read-the-feedback)

---

## Workspace layout

Put results in `<skill-name>-workspace/` as a sibling to the skill directory. Within the workspace, organize by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that, each test case gets a directory named descriptively (`eval-0-pdf-with-tables/`, not just `eval-0/`).
...
Read the benchmark data and surface patterns the aggregate stats might hide. See `agents/analyzer.md` (the "Analyzing Benchmark Results" section) for what to look for — assertions that always pass regardless of skill (non-discriminating), high-variance evals (possibly flaky), time/token tradeoffs.
...
Use `generate_review.py` to create the viewer; do not write custom HTML.
